...
local consoleSlotIndex, consoleSlotUseIndex, consoleSlotCount, consoleQuickSlotCursor
  ...
  for ii = 1, self.config.slotCount do
    do
      ...
      function slot:isEmpty()
        return self.item == nil and self.skill == nil
      end
...
function QuickSlot_UpdateData()
  ...
  for idx, slot in ipairs(self.slots) do
    ...
    if true == _ContentsGroup_isConsoleTest and true == getGamePadEnable() and idx == consoleSlotUseIndex and nil ~= slot.background then
...
function FGlobal_QuickSlot_Console_SetIndex(isRight)
  if nil ~= isRight then
...
function FGlobal_QuickSlot_Console_UseItem()
