...
function InitCustomizationMainUI()
  ...
  if initialized == false then
    createUI()
    initialized = true
    if nil ~= FGlobal_CashCustomization_ConsoleAdd then
  ...
  if CheckButton_ToggleUi:IsCheck() then
    ...
    elseif 1 == hideUIIndex then
      ...
      if InGameMode == true and Panel_Cash_Customization:IsShow() == false then
        ...
        if true == ToClient_isUIDATA2() then
...
function Panel_Customization_KeyGuideShow(isFirstOpen)
  Panel_CustomizationMessage:SetShow(false)
  if true == Panel_Customization_ConsoleKeyGuide:GetShow() then
    Panel_Customization_ConsoleKeyGuide:SetShow(false)
    return
  end
...
function CustomizationMain_SettingConsoleUI()
  for idx = 1, mainButtonNum do
    if idx == 2 then
      if 5 == mainButtonNum then
        Add_CustomizationUIGroup(0, 2, UI_GroupType.eCONSOLE_UI_CONTROL_TYPE_NOTEVENT, true)
