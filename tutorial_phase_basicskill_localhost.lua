PaGlobal_TutorialPhase_BasicSkill_LocalHost = {
  _phaseNo = 6,
  _currentType = 0,
  _currentStep = 0,
  _nextStep = 0,
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:checkPossibleForPhaseStart(stepNo)
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:checkSkippablePhase()
  if true == self._isSkippable and true == PaGlobal_TutorialManager:checkHaveOverLevelCharacter() then
    return true
  end
  return false
end
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:startPhase(stepNo, typeNo)
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:startPhaseXXX(stepNo, typeNo)
  PaGlobal_TutorialManager:setCurrentPhaseNo(self._phaseNo)
  PaGlobal_TutorialManager:setDoingTutorial(true)
  _PA_LOG("\234\179\189\235\175\188\236\154\176", "PaGlobal_TutorialPhase_BasicSkill_LocalHost:startStep() typeNo : " .. tostring(typeNo))
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:endPhase()
  self._currentStep = 0
  self._nextStep = 1
...
local result = false
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:updatePerFrame(deltaTime)
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:handleChangeStep(currentStep)
  if 1 == self._currentType then
    if 1 == self._currentStep then
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:showUiScoldingTooManyKeyInput()
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:changeStepTryBasicSkill()
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:updateTryBasicSkill(deltaTime)
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:eventCallUsedSkill_TryBasicSkill(skillWrapper)
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:changeStepTrySkillCombo()
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:updateTrySkillCombo(deltaTime)
  PaGlobal_TutorialUiManager:getUiKeyButton():ButtonToggleAll(true)
  if self._totalComboCount <= self._usedComboCount then
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:eventCallUsedSkill_TrySkillCombo(skillWrapper)
  local listNo = 0
  local skillNo = skillWrapper:getSkillNo()
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:changeStepSuggestCallBlackSpirit()
  if 1 == self._currentProgress then
    PaGlobal_TutorialUiManager:getUiBlackSpirit():showSuggestCallSpiritUi()
    PaGlobal_TutorialManager:setAllowCallBlackSpirit(true)
  end
end
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:eventCallAcceptBlackSpiritQuest(isAccept, questGroupNo, questId)
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:eventCallAfterBlackSpiritDialogClose()
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:handleEventQuestUpdateNotify(isAccept, questNoRaw)
...
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:handleAfterAndPopFlush()
  if 1 == self._currentType and 3 == self._currentStep then
    self:eventCallAfterBlackSpiritDialogClose()
  end
end
function PaGlobal_TutorialPhase_BasicSkill_LocalHost:handleEventSelfPlayerUsedSkill(skillWrapper)
