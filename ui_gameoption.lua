...
local frame_Game = {
  ...
  _btn_AutoRunCamera = UI.getChildControl(_frameContent_Game, "CheckButton_AutoRunCamera"),
  _slider_AutoRunCamera = UI.getChildControl(_frameContent_Game, "Slider_AutoRunCamera")
}
local _btn_AutoRunCamera_Slider = UI.getChildControl(frame_Game._slider_AutoRunCamera, "Slider_AutoOptimization_Button")
...
local chk_Option = {
  ...
  currentAutoRunCamera = true,
  currentAutoRunCameraRotation = 1,
  ...
  appliedAutoRunCamera = true,
  appliedAutoRunCameraRotation = 1,
  ...
  savedAutoRunCamera = true,
  savedAutoRunCameraRotation = 1
}
...
function Option_RegistEventHandler()
  ...
  frame_Game._btn_AutoRunCamera:addInputEvent("Mouse_LUp", "GameOption_CheckAutoRunCamera()")
  frame_Game._slider_AutoRunCamera:addInputEvent("Mouse_LPress", "GameOption_AutoRunCamera_Slider()")
  _btn_AutoRunCamera_Slider:addInputEvent("Mouse_LPress", "GameOption_AutoRunCamera_Slider_Btn()")
...
function GameOption_InitSystemNotify()
  ...
  for index = 0, 11 do
    initAlertType[index] = ToClient_GetMessageFilter(index)
  end
end
...
function GameOption_SetGameMode(gameOptionSetting)
  ...
  local AutoRunCamera = gameOptionSetting:getAutoRunCamera()
  local AutoRunCameraRotation = gameOptionSetting:getAutoRunCameraRotation()
  ...
  self.currentAutoRunCamera = AutoRunCamera
  self.appliedAutoRunCamera = AutoRunCamera
  self.savedAutoRunCamera = AutoRunCamera
  frame_Game._btn_AutoRunCamera:SetCheck(AutoRunCamera)
  self.currentAutoRunCameraRotation = AutoRunCameraRotation
  self.appliedAutoRunCameraRotation = AutoRunCameraRotation
  self.savedAutoRunCameraRotation = AutoRunCameraRotation
  GameOption_SetAutoRunCameraSlider(AutoRunCameraRotation)
end
...
function GameOption_CheckAutoRunCamera()
  local self = chk_Option
  if frame_Game._btn_AutoRunCamera:IsCheck() then
    self.currentAutoRunCamera = true
  else
    self.currentAutoRunCamera = false
  end
  GameOption_UpdateOptionChanged()
end
function GameOption_AutoRunCamera_Slider()
  local self = chk_Option
  local fValue = _btn_AutoRunCamera_Slider:GetPosX() / (frame_Game._slider_AutoRunCamera:GetSizeX() - _btn_AutoRunCamera_Slider:GetSizeX())
  self.currentAutoRunCameraRotation = fValue
  GameOption_UpdateOptionChanged()
end
function GameOption_AutoRunCamera_Slider_Btn()
  local self = chk_Option
  local fValue = _btn_AutoRunCamera_Slider:GetPosX() / (frame_Game._slider_AutoRunCamera:GetSizeX() - _btn_AutoRunCamera_Slider:GetSizeX())
  self.currentAutoRunCameraRotation = fValue
  GameOption_UpdateOptionChanged()
end
function GameOption_SetAutoRunCameraSlider(fValue)
  local self = chk_Option
  frame_Game._slider_AutoRunCamera:SetControlPos(fValue * 100)
end
...
function GameOption_Cancel()
  ...
  for index = 0, 11 do
    ToClient_SetMessageFilter(index, initAlertType[index])
  end
  gamePanel_Main._btn_Apply:SetEnable(false)
  SetUIMode(Defines.UIMode.eUIMode_Default)
  keyCustom_RollBack()
...
function GameOption_SaveSetting()
  PaGlobal_Panel_SaveSetting_Show(false)
end
function GameOption_Apply()
  ...
  if self.currentAutoRunCamera ~= self.appliedAutoRunCamera then
    self.appliedAutoRunCamera = self.currentAutoRunCamera
    self.savedAutoRunCamera = self.currentAutoRunCamera
    setAutoRunCamera(self.appliedAutoRunCamera)
  end
  if self.currentAutoRunCameraRotation ~= self.appliedAutoRunCameraRotation then
    self.appliedAutoRunCameraRotation = self.currentAutoRunCameraRotation
    self.savedAutoRunCameraRotation = self.appliedAutoRunCameraRotation
    setAutoRunCameraRotation(self.appliedAutoRunCameraRotation)
  end
...
function FGlobal_prevOption_luaLoadComplete()
  if isNeedGameOptionFromServer() == true then
    keyCustom_StartEdit()
  end
end
...
registerEvent("FromClient_luaLoadComplete", "FGlobal_prevOption_luaLoadComplete")
