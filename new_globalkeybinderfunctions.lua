...
function PaGlobal_GlobalKeyBinder.Process_UIMode_MiniGame(deltaTime)
  ...
  if GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_ESCAPE) or GlobalKeyBinder_CheckCustomKeyPressed(CppEnums.UiInputType.UiInputType_WorldMap) then
    FGlobal_CloseWorldmapForLuaKeyHandling()
  end
  if not getEscHandle() and GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_ESCAPE) then
    close_WindowPanelList()
...
function PaGlobal_GlobalKeyBinder.Process_UIMode_CommonWindow(deltaTime)
  ...
  if true == _ContentsGroup_isMemoOpen and GlobalKeyBinder_CheckCustomKeyPressed(CppEnums.UiInputType.UiInputType_Memo) then
...
function PaGlobal_GlobalKeyBinder.Process_ConsoleQuickMenu(deltaTime)
  if false == _ContentsGroup_isConsoleTest or false == getGamePadEnable() then
    return
  end
  if 0 == PaGlobal_Option:Get("ConsolePadKeyType") then
    return
  end
  ...
  if false == FGlobal_ConsoleQuickMenu_IsShow() then
