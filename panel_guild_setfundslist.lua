...
local _guildFoundationValue = UI.getChildControl(Panel_Guild_SetFundsList, "StaticText_GuildMoney")
local _btnSetAllFunds = UI.getChildControl(Panel_Guild_SetFundsList, "Button_SetAllFunds")
local _editAllFunds = UI.getChildControl(Panel_Guild_SetFundsList, "Edit_InputAllFunds")
local _checkAllUnlimited = UI.getChildControl(Panel_Guild_SetFundsList, "CheckButton_FundsLimit")
...
local Guild_Funds = {
  ...
  _memberFundsButton = UI.getChildControl(Panel_Guild_SetFundsList, "Button_C_SetFunds"),
...
Guild_Funds._memberFundsButton:SetShow(false)
...
function Guild_Funds:ResetControl()
  ...
  for index = 1, _memberCtrlCount do
    ...
    _guildList[index]._memberFundsButton = UI.createControl(UCT.PA_UI_CONTROL_BUTTON, _contentGuildList, "Button_memberFundsButton_" .. index - 1)
    ...
    CopyBaseProperty(Guild_Funds._memberFundsButton, _guildList[index]._memberFundsButton)
    ...
    _guildList[index]._memberFundsButton:SetPosY(posY)
    _guildList[index]._memberFundsButton:SetShow(true)
...
function Guild_Funds:UpdateData()
  ...
  for index = 1, memberCount do
    ...
    if 0 == gradeType then
      ...
      _guildList[index]._memberFundsButton:SetIgnore(true)
      _guildList[index]._memberFundsButton:SetMonoTone(true)
    else
      ...
      _guildList[index]._memberFundsButton:addInputEvent("Mouse_LUp", "SetFundsForOneMember(" .. index .. ")")
      _guildList[index]._memberFundsButton:SetIgnore(false)
      _guildList[index]._memberFundsButton:SetMonoTone(false)
    end
  end
...
function SetTempFundsOfMemberCallback(fundsValue, index)
  ...
  if fundsValue == guildMemberInfo:getPriceLimit() then
    _guildList[index]._memberFundsLimitEdit:SetText("<PAColor0xffefefef>" .. tostring(fundsValue) .. "<PAOldColor>")
  else
    _guildList[index]._memberFundsLimitEdit:SetText("<PAColor0xff00b4ff>" .. tostring(fundsValue) .. "<PAOldColor>")
  end
...
function ToggleLimitOfMember(index)
  local dataIdx = _guildMemberList[index].idxFromServer
  local guildMemberInfo = _guildInfo:getMember(dataIdx)
  local isUnlimited = not guildMemberInfo:getIsPriceLimit()
  if _guildList[index]._memberFundsIsUnlimited:IsCheck() then
    ...
    if isUnlimited then
      _guildList[index]._memberFundsLimitEdit:SetText("<PAColor0xff00b4ff>" .. tostring(makeDotMoney(guildMemberInfo:getPriceLimit())) .. "<PAOldColor>")
    else
      _guildList[index]._memberFundsLimitEdit:SetText("<PAColor0xffefefef>" .. tostring(makeDotMoney(guildMemberInfo:getPriceLimit())) .. "<PAOldColor>")
    end
    _guildList[index]._memberFundsLimitEdit:SetIgnore(false)
  end
end
function SetFundsForOneMember(index)
  ...
  local dataIdx = _guildMemberList[index].idxFromServer
  local guildMemberInfo = _guildInfo:getMember(dataIdx)
  local isUnlimited = _guildList[index]._memberFundsIsUnlimited:IsCheck()
  if true == isUnlimited then
...
function Guild_Funds_SetAllFunds()
  ...
  local fundsVal = _editAllFunds:GetText()
  for i = 0, _guildInfo:getMemberCount() - 1 do
    local guildMemberInfo = _guildInfo:getMember(i)
    if 0 ~= guildMemberInfo:getGrade() then
      if true == _checkAllUnlimited:IsCheck() and guildMemberInfo:getIsPriceLimit() then
        ToClient_SetGuildMemberPriceLimit(i, 0, false)
      elseif fundsVal ~= tostring(guildMemberInfo:getPriceLimit()) then
        ToClient_SetGuildMemberPriceLimit(i, fundsVal, true)
      elseif false == _checkAllUnlimited:IsCheck() and not guildMemberInfo:getIsPriceLimit() and "0" == fundsVal then
        ToClient_SetGuildMemberPriceLimit(i, fundsVal, true)
      end
    end
  end
